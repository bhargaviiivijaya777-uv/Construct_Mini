...
if(isset($_POST['submit'])){
    ...
    if($conn->query("UPDATE materials SET material_name='$name', price=$price, quantity='$qty', unit='$unit', seller_id=$seller_id WHERE material_id=$id")){
...
<head>
<meta charset="utf-8">
<title>Edit Material - CONSTRUCTHUB</title>
...
<main class="container">
    ...
    <div class="form-container">
        ...
        <form method="post">
            <input name="material_name" placeholder="Material Name" value="<?php echo htmlspecialchars($row['material_name']); ?>" required>
            <input name="price" type="number" step="0.01" placeholder="Price" value="<?php echo $row['price']; ?>" required>
            
            <div class="quantity-row">
                <input name="quantity" placeholder="Quantity (e.g., 50, 1000, 25 kgs)" value="<?php echo htmlspecialchars($row['quantity']); ?>" required>
                <select name="unit">
                    <option value="units" <?php if($row['unit']=='units') echo 'selected'; ?>>Units</option>
                    <option value="kg" <?php if($row['unit']=='kg') echo 'selected'; ?>>Kilograms</option>
                    <option value="g" <?php if($row['unit']=='g') echo 'selected'; ?>>Grams</option>
                    <option value="tons" <?php if($row['unit']=='tons') echo 'selected'; ?>>Tons</option>
                    <option value="feet" <?php if($row['unit']=='feet') echo 'selected'; ?>>Feet</option>
                    <option value="meters" <?php if($row['unit']=='meters') echo 'selected'; ?>>Meters</option>
                    <option value="sq ft" <?php if($row['unit']=='sq ft') echo 'selected'; ?>>Square Feet</option>
                    <option value="sq m" <?php if($row['unit']=='sq m') echo 'selected'; ?>>Square Meters</option>
                    <option value="bags" <?php if($row['unit']=='bags') echo 'selected'; ?>>Bags</option>
                    <option value="pieces" <?php if($row['unit']=='pieces') echo 'selected'; ?>>Pieces</option>
                    <option value="bundles" <?php if($row['unit']=='bundles') echo 'selected'; ?>>Bundles</option>
                    <option value="cubic feet" <?php if($row['unit']=='cubic feet') echo 'selected'; ?>>Cubic Feet</option>
                    <option value="cubic meters" <?php if($row['unit']=='cubic meters') echo 'selected'; ?>>Cubic Meters</option>
                    <option value="liters" <?php if($row['unit']=='liters') echo 'selected'; ?>>Liters</option>
                    <option value="custom" <?php if($row['unit']=='custom') echo 'selected'; ?>>Custom</option>
                </select>
            </div>
            ...
            <button name="submit" type="submit"><i class="fas fa-save"></i> Update Material</button>
        </form>
    </div>

    <a href="view_materials.php" class="back-link">
